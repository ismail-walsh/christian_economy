...
class ChristianEconomySupabaseUser extends BaseAuthUser {
  ChristianEconomySupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> christianEconomySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ChristianEconomySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
